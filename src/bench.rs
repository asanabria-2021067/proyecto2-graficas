...
const VIEWS: [(f32, f32, f32); 3] = [(35.0, 25.0, 60.0), (120.0, 55.0, 45.0), (250.0, 15.0, 90.0)];
const FRAMES_PER_VIEW: usize = 10;

pub fn run(tracer: &dyn Tracer, width: u32, height: u32) {
    ...
    for &(yaw, pitch, dist) in VIEWS.iter() {
        let cam = Camera::new(center, yaw, pitch, dist, 5.0, 300.0, 50.0);
